...
export const Ink = ({ annotation, page, viewport }) => {
    ...
    return (React.createElement(Annotation, { annotation: annotation, hasPopup: hasPopup, ignoreBorder: true, isRenderable: isRenderable, page: page, viewport: viewport }, (props) => (React.createElement("div", Object.assign({}, props.slot.attrs, { className: styles.annotation, "data-annotation-id": annotation.id, "data-annotation-type": "ink", onClick: props.popup.toggleOnClick, onMouseEnter: props.popup.openOnHover, onMouseLeave: props.popup.closeOnHover }),
        annotation.inkLists && annotation.inkLists.length && (React.createElement("svg", { height: `${height}px`, preserveAspectRatio: "none", version: "1.1", viewBox: `0 0 ${width} ${height}`, width: `${width}px` }, annotation.inkLists.map((inkList, index) => (React.createElement("polyline", { key: index, fill: "none", stroke: "transparent", strokeWidth: borderWidth || 1, points: inkList.map((item) => `${item.x - rect[0]},${rect[3] - item.y}`).join(' ') }))))),
        props.slot.children))));
};
